...
const Auth = (props) => {
  ...
  const Submit = async () => {
    ...
    setisloading(true);
    await fetch(
      "https://identitytoolkit.googleapis.com/v1/accounts:signUp?key=AIzaSyCmvp3oT5_K82HUaNPsIQ9S4E7_r0M2gfU",
      {
        method: "POST",
        body: JSON.stringify({
          email: Email,
        ...
        if (res.ok) {
          return res.json().then((data) => {
            console.log(data),
              ...
              ); //Milliseconds conversion foe new date object for expiration of async token in storage
          });
        } else {
    ...
    <View style={styles.auth}>
      ...
      <View style={styles.buttons}>
        {isloading ? (
          ...
          <Button title="signup" color={Colors.primary} onPress={Submit} />
        )}
        <TouchableOpacity
          style={styles.signin}
          onPress={() => {
